...
export default function IconAlexandre({ position }) {
  const iconRef = useRef();
  const [cardCocktail, setCardCocktail] = useState(false);

  useEffect(() => {
    const loader = new GLTFLoader();
    loader.load("./public/gltf/alex.glb", (gltf) => {
  ...
  return (
    <group scale={2.8} dispose={null} position={position}>
      ...
      {cardCocktail && (
        <Html>
          <CocktailCard cocktailId="11009" onClose={hideCard} />
        </Html>
      )}
    </group>
  );
