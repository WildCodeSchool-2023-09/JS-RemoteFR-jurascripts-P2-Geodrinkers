...
import cardsDataMap from "../../datas/CardsDataMap";

export default function IconJL({ position }) {
  const iconRef = useRef();
  const [cardCocktail, setCardCocktail] = useState(false);

  useEffect(() => {
    const loader = new GLTFLoader();
    loader.load("./src/assets/gltf/jl.glb", (gltf) => {
            ...
            <div className="card">
              ...
              <div className="card-header">
                <div className="img-ctn">
                  <img
                    src={cardsDataMap[0].picture}
                    alt={cardsDataMap[0].name}
                  />
                </div>
              </div>
              <h1 className="title">{cardsDataMap[0].title}</h1>
              <div className="description">{cardsDataMap[0].description}</div>
            </div>
          </div>
        </Html>
      )}
