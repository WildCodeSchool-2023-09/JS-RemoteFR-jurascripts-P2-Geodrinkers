...
export default function IconSandrine({ position }) {
  const iconRef = useRef();
  const [cardCocktail, setCardCocktail] = useState(false);

  useEffect(() => {
    const loader = new GLTFLoader();
    loader.load("./public/gltf/sandrine.glb", (gltf) => {
  ...
  return (
    <group scale={2.8} dispose={null} position={position}>
      ...
      {cardCocktail && (
        <Html>
          <CocktailCard cocktailId="11666" onClose={hideCard} />
        </Html>
      )}
    </group>
  );
